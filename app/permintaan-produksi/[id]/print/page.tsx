               ...
               {request.spk && request.spk.spkItems && request.spk.spkItems.length > 0 ? (
                  <div className="space-y-2 mt-1">
                      {request.spk.spkItems.map((item, idx) => (
                          <div key={idx} className="text-sm border-b border-gray-200 last:border-0 pb-1">
                              <div className="font-semibold">{item.qty} {item.satuan} - {item.namaBarang}</div>
                              {item.salesOrder?.spesifikasi_barang && (
                                  <div className="text-xs text-gray-600 italic">Spec: {item.salesOrder.spesifikasi_barang}</div>
                              )}
                          </div>
                      ))}
                  </div>
